  ...
  return (
    <div className="container">
      ...
      <p style={{ color: "var(--text-muted)", fontSize: 14 }}>
        ...
        automatically creates a work order â€” see the README for how to wire up the cron job.
      </p>
      <form onSubmit={handleSubmit} className="card" style={{ marginBottom: 16, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
        ...
        <div>
          ...
          <select value={form.assetId} onChange={(e) => setForm({ ...form, assetId: e.target.value })}>
            ...
            {assets.map((a) => <option key={a.id} value={a.id}>{a.name} â€” {a.site?.name}</option>)}
          </select>
        </div>
      ...
      <table>
        ...
        <tbody>
          {schedules.map((s) => (
            <tr key={s.id}>
              <td>{s.name}</td><td>{s.asset.name}</td><td>{s.asset.site?.name || "â€”"}</td><td>{s.frequencyDays} days</td>
